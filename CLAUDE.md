...
- **Local venv:** `/workspace/spacetime-sls/.venv` (Python 3.14.5). Installed and importing:
  `numpy 2.5.0, pandas 3.0.4, matplotlib 3.11.0, astropy 8.0.0, pytest 9.1.1, h3 4.5.0,
  sgp4, skyfield`. Use `.venv/bin/python` / `.venv/bin/pytest` to build and test locally.
  Add deps with `.venv/bin/pip install ...` (works; pypi is reachable).
- **The system interpreter `/usr/bin/python3` has no stack** — always use the venv.
