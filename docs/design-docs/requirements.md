...
| SA13 | **Make-before-break handover continuity**: continuous k=1 + mandatory 2-sat overlap ≥ τ_overlap. **LIVE end-to-end** (`run_coverage_h3(continuity_overlap_s=…, require_different_plane=…)`): grid-quantized detection (±step_s), different-**sat** + different-**plane** serving-path gate, merged-interval worst-gap (`mbb_worst_gap_s`), Nyquist precondition, resolution metadata (`detection_step_s`, `refine_tol_s`). **Deferred**: sub-second endpoint refinement (`coverage/refine.py`); `SimConfig` knobs `max_buffered_gap_s`, `min_target_dwell_s`, `max_handover_rate_hz`, buffered-break (inert, no consumer); full §6 event bracketing, rigorous k≥2 continuum verdict, worst link margin → Slice B. Promotes FUT4 | A | ◐ |
